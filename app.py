from flask import Flask, render_template, request, redirect, url_for, flash, session, send_file, make_response, render_template
...
def admin_dashboard():
    ...
    # Calculate sold quantity for each item
    items_with_sold_quantity = [(item, item.initial_quantity - item.quantity) for item in items]
    
    return render_template('admin_dashboard.html', items=items_with_sold_quantity, filter_type=filter_type, sort_by=sort_by)
